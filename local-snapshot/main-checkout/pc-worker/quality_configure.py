...
VOCAL_RULE = 'Insufficient vocal signal activity'
VOCAL_GUARD = ast.parse('float(mask.mean())>=.50', mode='eval').body


def compile_policy(source, filename):
    tree = ast.parse(source, filename)
    changed = {'break': 0, 'intro': 0, 'vocal': 0}

    class Policy(ast.NodeTransformer):
        def visit_Assert(self, node):
            nonlocal changed
            if (isinstance(node.msg, ast.Tuple) and len(node.msg.elts) == 2
                    and isinstance(node.msg.elts[0], ast.Constant) and node.msg.elts[0].value == VOCAL_RULE
                    and ast.dump(node.test) == ast.dump(VOCAL_GUARD)):
                changed['vocal'] += 1
                node.test = ast.BoolOp(op=ast.Or(), values=[node.test, ast.Call(
                    func=ast.Name(id='_distonyc_accept_sparse_intent', ctx=ast.Load()),
            ...
            if (isinstance(node.msg, ast.Tuple) and len(node.msg.elts) == 2
                ...
                changed['break'] += 1
                # Retain the minimum vocal span unless the exact hash-pinned sparse-intent
                # policy accepts it. The instrumental-gap upper bound remains advisory.
                span_test = ast.BoolOp(op=ast.Or(), values=[node.test.values[0], ast.Call(
                    func=ast.Name(id='_distonyc_accept_sparse_intent', ctx=ast.Load()),
                    args=[ast.Name(id='evidence', ctx=ast.Load())], keywords=[])])
                span = ast.copy_location(ast.Assert(test=span_test, msg=node.msg), node)
    ...
    if changed['break'] != 1 or changed['intro'] != 1 or changed['vocal'] > 1:
...
def configure(work, expected_sha):
    ...
    issues = []
    sparse_intent = {'accepted': False}

    def accept_sparse_intent(evidence):
    ...
    try:
        ...
        exec(code, {'__name__': '__main__', '__file__': str(source),
                    ...
                    '_distonyc_accept_sparse_intent': accept_sparse_intent})
    finally: sys.argv = previous
    save(work / 'arrangement-quality-policy.json', {'version': 1, 'source_sha256': expected_sha,
         'qualityIssues': issues, 'operator_sparse_intent': sparse_intent['accepted'],
         'other_integrity_checks_retained': True, 'integrity_checks_retained': True,
         'original_checker_unchanged': True})
