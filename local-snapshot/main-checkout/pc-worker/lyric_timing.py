...
def valid_cues(cues, duration):
    ...
    for cue in cues:
        if not isinstance(cue, dict): return False
        ...
        if (not isinstance(start, (int, float)) or isinstance(start, bool) or not math.isfinite(start)
                ...
                or not 0 <= start < end <= duration + .01 or start <= previous_start):
            return False
...
def make_cues(work, text, duration, heard=None):
    ...
    for index, line in enumerate(singable):
        ...
        previous_last = last
    cues = spread_collisions(cues, duration)
    return cues if valid_cues(cues, duration) else []
