...
class CompositionEndingTests(unittest.TestCase):
    ...
    def test_sub_half_spoken_jazz_coverage_requires_exact_intent_policy(self):
        with tempfile.TemporaryDirectory() as directory:
            request = self.request(Path(directory))
            work = self.prepare_evidence(request, outro=8.34, coverage=.3927)
            with self.assertRaisesRegex(ValueError, 'Invalid composition-ending measurements'):
                evidence(work)
            save(work / 'sparse-vocal-intent-policy.json', {'accepted': True})
            with patch('sparse_intent_policy.verify', return_value={'accepted': True}) as verify:
                measured = evidence(work)
            self.assertTrue(measured['operator_sparse_intent'])
            self.assertAlmostEqual(measured['post_vocal_seconds'], 8.34)
