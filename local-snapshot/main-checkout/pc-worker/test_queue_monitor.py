...
from queue_monitor import POLICY_VERSION, classify, retry_budget, scan
...
class MonitorTests(unittest.TestCase):
    ...
    def test_deterministic_spacing_gets_one_policy_attempt(self):
        with tempfile.TemporaryDirectory() as root, patch('queue_monitor.local_context',return_value={'has_request':True}):
            ...
            self.assertEqual(classify('Long instrumental introduction', {})[:2], ('musical_spacing', 'retry'))
    def test_exhausted_sparse_provider_policy_fallback_gets_one_new_policy_attempt(self):
        provider = ('Eleven Music HTTP 400 [bad_request]: composition plan violated Terms of Service. '
        ...
        with tempfile.TemporaryDirectory() as root:
            ...
            original = Path(root) / 'original'; original.mkdir()
            (original / 'sparse-vocal-intent-policy.json').write_text('{}')
            (directory / 'render-request.json').write_text('{}')
            save(directory / 'sparse-vocal-repair.json', {
                'status': 'failed', 'attempt_limit': 1, 'attempts': 1,
                'original_work': str(original), 'error': provider})
            context = {'has_request': True, 'directory': str(directory)}
            self.assertEqual(classify(provider, context)[:2], ('sparse_vocal', 'retry'))
            with patch('queue_monitor.local_context', return_value=context):
                api = FakeAPI([prompt(error=provider)]); cfg = self.config(root)
                scan(cfg, api, now=0)
                api.rows[0]['status'] = 'failed'
                scan(cfg, api, now=99999)
            self.assertEqual(api.calls, ['one'])
            attempts = load(Path(root) / 'monitor/ledger.json')['requests']['one']['attempts']
            self.assertEqual([(row['category'], row['policy_version']) for row in attempts],
                             [('sparse_vocal', POLICY_VERSION)])

            (original / 'sparse-vocal-intent-policy.json').unlink()
            self.assertEqual(classify(provider, context)[:2], ('creative_plan', 'review'))
            self.assertEqual(classify('Temporary connection failure', context)[:2],
                             ('transient_runtime', 'retry'))

    def test_sparse_budget_is_new_per_policy_but_does_not_change_other_budgets(self):
        old = {'attempts': [{'category': 'sparse_vocal',
                             'policy_version': POLICY_VERSION - 1}]}
        self.assertTrue(retry_budget(old, 'sparse_vocal'))
        current = {'attempts': old['attempts'] + [{
            'category': 'sparse_vocal', 'policy_version': POLICY_VERSION}]}
        self.assertFalse(retry_budget(current, 'sparse_vocal'))
        self.assertTrue(retry_budget(current, 'transient_runtime'))
    ...
    def test_verified_export_goes_to_publication_and_voice_attempt_is_not_repeated(self):
        ...
        with tempfile.TemporaryDirectory() as root:
            packaging = 'Mastering the complete song failed: command returned non-zero exit status 1'
            self.assertEqual(classify(packaging, {'directory': root})[:2], ('unknown', 'review'))
            Path(root, 'suite-finisher-repair.json').write_text('{}')
            self.assertEqual(classify(packaging, {'directory': root})[:2], ('suite_packaging', 'retry'))
        with tempfile.TemporaryDirectory() as root:
            context = {'voice_model':'v8','state':{'stage':'assemble'},'directory':root}
            self.assertEqual(classify('assert active.any()',context)[:2],('unknown','review'))
            Path(root, 'inactive-voice-repair-approved.json').write_text('{}')
            self.assertEqual(classify('assert active.any()',context)[:2],('inactive_voice','retry'))
            Path(root, 'inactive-voice-repair-approved.json').unlink()
            Path(root, 'sparse-vocal-repair.json').write_text('{}')
            self.assertEqual(classify('assert regions', {'directory': root})[:2], ('sparse_vocal', 'retry'))
            nested = Path(root, 'movements', '1'); nested.mkdir(parents=True)
            Path(root, 'sparse-vocal-repair.json').unlink()
            Path(nested, 'sparse-vocal-repair.json').write_text('{}')
            movement_error = 'Movement 1 of 3 - Checking the arrangement failed'
            self.assertEqual(classify(movement_error, {'directory': root})[:2], ('sparse_vocal', 'retry'))
            movement2_error = 'Movement 2 of 3 - Checking the arrangement failed'
            self.assertEqual(classify(movement2_error, {'directory': root})[:2], ('unknown', 'review'))
            second = Path(root, 'movements', '2'); second.mkdir()
            Path(second, 'sparse-vocal-intent-approved.json').write_text('{}')
            self.assertEqual(classify(movement2_error, {'directory': root})[:2], ('sparse_vocal', 'retry'))
            ending_error = 'Movement 2 of 3 - Preparing voice sections failed: Invalid composition-ending measurements'
            self.assertEqual(classify(ending_error, {'directory': root})[:2], ('sparse_vocal', 'retry'))
            self.assertEqual(classify('Movement 3 of 3 - Checking the arrangement failed', {'directory': root})[:2], ('unknown', 'review'))
            inactive = nested / 'sparse-vocal-attempt'; inactive.mkdir()
            Path(inactive, 'inactive-voice-repair-approved.json').write_text('{}')
            movement_assembly = 'Movement 1 of 3 - Assembling the lead failed'
            nested_context = {'voice_model': 'v8', 'directory': root}
            self.assertEqual(classify(movement_assembly, nested_context)[:2], ('inactive_voice', 'retry'))
            self.assertEqual(classify('Movement 2 of 3 - Assembling the lead failed', nested_context)[:2], ('unknown', 'review'))

        versioned = {'has_request': True, 'voice_model': 'v7', 'vocal_warnings_enabled': True,
                     'state': {'stage': 'finish'}}
        self.assertIn('not supported', classify('Missing vocal phrase', versioned)[2])
