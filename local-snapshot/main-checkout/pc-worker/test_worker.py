...
class WorkerTests(unittest.TestCase):
    ...
    def test_lyrics_use_frozen_render_inputs_and_preserve_export(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            save(work / 'matched-vocals-words.json', [{'words': [
                {'word': 'Actual', 'start': 2.1, 'end': 2.6}, {'word': 'saved', 'start': 2.6, 'end': 3.0},
                {'word': 'words', 'start': 3.0, 'end': 3.5}]}])
            timed = make_sheet(config, plan(), {'work_path': str(work), 'duration': 10})
            self.assertEqual(timed['cues'], [{'line': 1, 'start': 2.1, 'end': 3.5}])
