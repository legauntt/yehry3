...
class DurationTests(unittest.TestCase):
    ...
    def test_warning_rollup_preserves_flag_only_quality_notices(self):
        reports = [{'title': str(i), 'duration': 220, 'final_post_vocal_seconds': 8,
                    'arrangement': {'first_detected_voice': 3},
                    'qualityIssues': [{'code': 'unconfirmed_lyric_ending'},
                                      {'code': 'long_instrumental_break', 'seconds': 12 + i}]}
                   for i in range(3)]
        issues, _ = aggregate_issues(reports)
        self.assertEqual(issues, [{'code': 'unconfirmed_lyric_ending'},
                                  {'code': 'long_instrumental_break', 'seconds': 14}])
