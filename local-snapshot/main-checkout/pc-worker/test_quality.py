...
from quality_finish import compile_policy, review_outro, warn_vocals, validate_mp3_delivery, RULE
...
from quality_verify import adapt as adapt_verification, warning_is_valid


class QualityTests(unittest.TestCase):
    def test_mp3_delivery_accepts_only_small_silent_positive_padding(self):
        import numpy as np
        mix = np.zeros((44100, 2), dtype='float32')
        exact = mix.copy()
        padded = np.concatenate([mix, np.zeros((29, 2), dtype='float32')])
        self.assertTrue(validate_mp3_delivery(exact, mix, .8, 44100))
        self.assertTrue(validate_mp3_delivery(padded, mix, .8, 44100))
        padded[-1, 0] = 2e-6
        self.assertFalse(validate_mp3_delivery(padded, mix, .8, 44100))
        self.assertFalse(validate_mp3_delivery(np.zeros((45253, 2)), mix, .8, 44100))
        self.assertFalse(validate_mp3_delivery(mix[:-3], mix, .8, 44100))
        self.assertFalse(validate_mp3_delivery(exact, mix, 1., 44100))

    def test_mp3_delivery_guard_is_adapted_without_changing_other_assertions(self):
        source = f'''assert 0<post_vocal_seconds<=13,({RULE!r},post_vocal_seconds)
assert np.isfinite(decoded).all() and abs(len(decoded)-len(mix))<=2 and encoded_peak<10**(-.5/20)
assert other_guard
exported=True
'''
        code = compile_policy(source, 'finisher.py')
        values = {'post_vocal_seconds': 3, 'decoded': object(), 'mix': object(),
                  'encoded_peak': .8, 'SR': 44100, 'other_guard': True,
                  '_distonyc_review_outro': lambda value: None,
                  '_distonyc_validate_mp3_delivery': lambda *args: True}
        exec(code, values)
        self.assertTrue(values['exported'])
        with self.assertRaises(AssertionError):
            exec(code, {**values, '_distonyc_validate_mp3_delivery': lambda *args: False})
        with self.assertRaises(AssertionError): exec(code, {**values, 'other_guard': False})

    def test_finisher_tolerates_only_sample_roundoff_at_ending_boundary(self):
        source = f'''assert end/SR>=effective_last_voice+2
...
'''
        ...
        values = {'end': end, 'SR': 44100, 'effective_last_voice': last,
                  'post_vocal_seconds': 2, '_distonyc_review_outro': lambda value: None}
        exec(code, dict(values))
        with self.assertRaises(AssertionError):
            exec(code, {**values, 'end': end - 1})
...
"""
    ...
    def test_versioned_finisher_remains_selected_and_hash_pinned(self):
        manifest = {'kind': 'new', 'workers': {'finish_versioned.py': 'versioned-hash'}, 'tasks': [
            {'name': 'finish', 'command': ['python', 'saved/finish_versioned.py', '--work', 'saved']}]}
        command = execution_manifest(manifest, vocal_dropout_warnings=True)['tasks'][0]['command']
        self.assertEqual(command[command.index('--source-name') + 1], 'finish_versioned.py')
        self.assertEqual(command[command.index('--source-sha256') + 1], 'versioned-hash')

    def test_delivery_verifier_relaxes_only_pinned_dropout_guard(self):
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            for name in ('selected-vocals.wav', 'selected-backing.wav', 'matched-vocals.wav'):
                (work / name).write_bytes(name.encode())
            save(work / 'vocal-quality-policy.json', {'version': 2,
                'recovery_disposition': 'not_supported_for_voice_model', 'audio_changed': False,
            ...
            save(work / 'mix-results.json', {'longest_missing_vocal_run_seconds': .6,
                'qualityIssues': [{'code': 'vocal_dropout', 'seconds': .6}]})
            ...
            class Engine:
                ...
                @staticmethod
                def verify_work(path, output_dir=None):
                    Engine.need(False, 'A vocal phrase may be missing.')
                    Engine.need(output_dir == 'valid', 'A later integrity check failed.')
                    return {'work_path': str(path)}

            adapt_verification(Engine)
            self.assertEqual(Engine.verify_work(work, 'valid')['work_path'], str(work))
            with self.assertRaisesRegex(ValueError, 'later integrity'):
                Engine.verify_work(work, 'invalid')
...
"""
        ...
        def run(**changes):
            ...
            values = {'voice_ok': True, 'first': 9.96, 'last': 239.54, 'duration': 269.2,
                      ...
                      '_distonyc_accept_sparse_intent': lambda evidence: False, **changes}
            exec(code, values)
            self.assertTrue(values['passed'])
            return issues
        self.assertEqual(run(), [{'code': 'long_instrumental_break', 'seconds': 12.32}])
        self.assertEqual(run(first=21.84), [{'code': 'long_instrumental_break', 'seconds': 21.84}])
        ...
        with self.assertRaises(ValueError): compile_arrangement(source.replace('first<13', 'first<20'), 'changed.py')

    def test_sparse_intent_changes_only_the_named_coverage_guard(self):
        source = """assert float(mask.mean())>=.50,('Insufficient vocal signal activity',evidence)
assert first<13,('Long instrumental introduction',evidence)
assert last-first>duration*.6 and max([g['seconds'] for g in gaps],default=0)<9.5,('Too much instrumental space',evidence)
assert ending_ok, 'Ending failed'
passed=True
"""
        code = compile_arrangement(source, 'configure.py')
        values = {'mask': type('Mask', (), {'mean': lambda self: .22})(), 'evidence': {},
                  'first': 5, 'last': 90, 'duration': 100, 'gaps': [], 'ending_ok': True,
                  '_distonyc_review_breaks': lambda gaps: None,
                  '_distonyc_review_intro': lambda first: None,
                  '_distonyc_accept_sparse_intent': lambda evidence: False}
        with self.assertRaisesRegex(AssertionError, 'Insufficient vocal'): exec(code, dict(values))
        accepted = {**values, '_distonyc_accept_sparse_intent': lambda evidence: True}
        exec(code, accepted)
        self.assertTrue(accepted['passed'])
        sparse_span = {**accepted, 'first': 25, 'last': 66, 'duration': 100}
        exec(code, sparse_span)
        self.assertTrue(sparse_span['passed'])
        with self.assertRaisesRegex(AssertionError, 'Ending failed'):
            exec(code, {**accepted, 'ending_ok': False})
