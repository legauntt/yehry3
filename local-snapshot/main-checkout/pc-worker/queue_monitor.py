...
def prepared_sparse_repair(error, context):
    directory = Path(context.get('directory', ''))
    if (directory / 'sparse-vocal-repair.json').is_file(): return True
    movement = re.search(r'movement (\d+) of \d+', error)
    if not movement: return False
    movement_dir = directory / 'movements' / movement.group(1)
    return any((movement_dir / name).is_file() for name in
               ('sparse-vocal-repair.json', 'sparse-vocal-intent-approved.json'))


def prepared_sparse_retained_fallback(error, context):
    error = error.casefold()
    directory = Path(context.get('directory', ''))
    journal = directory / 'sparse-vocal-repair.json'
    if (not journal.is_file() or not (directory / 'render-request.json').is_file()
            or 'eleven music http 400' not in error or 'terms of service' not in error
            or 'no automatic repeat charge' not in error):
        return False
    try:
        record = load(journal)
        from sparse_vocal_repair import provider_policy_rejection
        original = Path(record.get('original_work', ''))
        return (provider_policy_rejection(record)
                and (original / 'sparse-vocal-intent-policy.json').is_file())
    except (OSError, ValueError, TypeError):
        return False


def prepared_inactive_repair(error, context):
    directory = Path(context.get('directory', ''))
    if (directory / 'inactive-voice-repair-approved.json').is_file(): return True
    movement = re.search(r'movement (\d+) of \d+.*assembling the lead', error)
    if not movement: return False
    movement_dir = directory / 'movements' / movement.group(1)
    return any((movement_dir / suffix / 'inactive-voice-repair-approved.json').is_file()
               for suffix in ('', 'sparse-vocal-attempt'))
...
def classify(error, context):
    ...
    if ('mastering the complete song' in error and 'returned non-zero exit status' in error
            and (Path(context.get('directory', '')) / 'suite-finisher-repair.json').is_file()):
        return 'suite_packaging', 'retry', 'Reuse all verified movements and rerun the repaired suite finisher.'
    ...
    if prepared_sparse_retained_fallback(error, context):
        return 'sparse_vocal', 'retry', 'Reuse the hash-pinned paid original after the exhausted replacement was rejected by provider policy.'
    ...
    if (('insufficient vocal signal activity' in error or 'assert regions' in error
            or 'invalid composition-ending measurements' in error
            or ('movement ' in error and 'checking the arrangement' in error))
            and prepared_sparse_repair(error, context)):
        return 'sparse_vocal', 'retry', 'Run the prepared one-time vocal-arrangement recovery with frozen inputs.'
    if (context.get('voice_model') == 'v8' and prepared_inactive_repair(error, context)
            and (('assert active.any()' in error and context.get('state', {}).get('stage') == 'assemble')
                 or ('movement ' in error and 'assembling the lead' in error))):
...
def retry_budget(entry, category):
    ...
    if category in ('vocal_dropout','sparse_vocal','unfinished_ending','musical_spacing','planner_format','unicode_runtime','shepherd'):
