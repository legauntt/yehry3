...
The isolated V8 candidate adds explicit musical controls, owner lyric approval, bounded composition choices and browser preferences. See [V8 implementation and validation](pc-worker/V8-GENERATION.md). The generation release is separate from the Tony voice checkpoint. API `YEHRY3_GENERATION_V8=true` and worker `generation_v8=true` are required for the generator. Tony V8 selection additionally requires API `YEHRY3_VOICE_V8=true` and a verified `voice_models.v8` worker profile; V7 remains the default. Requests preserve separate voice and generation identities; the coordinated rollout is controlled by these feature flags. Local preview submissions use a disposable database.
