...
- The same preferences include **Lyric captions**, enabled by default, which controls the record's comic-book lyric captions; **Continuous record spins**, which keeps the hero record turning without friction; and **Spin while music plays**, which follows playback and stops on pause unless continuous spinning is enabled. They apply immediately and persist across reloads, navigation, and tabs. Clicking still speeds up the record. Explicit spin choices also work with reduced motion enabled; the default idle spins remain disabled there. New preferences give the cog a brief gentle pulse and a **New** badge until opened, remembered in this browser; reduced motion uses the badge alone.
