...
- [The public queue](https://yehry3.app/queue/) shows active production, Needs Attention requests with retained work, waiting requests in priority order (50 per page), and the ten latest completed songs. Every confirmed item links to a stable public details URL. It refreshes every 30 seconds and on returning to the tab. Drafts, admin notes, worker errors, credentials, and local paths remain private. The request form explains that confirmed ideas and progress are public.
