...
The three **Tony V7 Study** songs are experimental listening material. V8 is the default voice for new requests when available and remains experimental; V6 and V7 are still available. New requests fall back to V7 when V8 is unavailable. Saved requests retain their selected voice. Telephone Wire retains two brief vocal gaps around 2:16 and 2:18 (1.6 seconds total); The Last Light in the Station retains its complete 26.54-second instrumental outro. Their catalog issue notices remain visible. Technical verification is not a human listening judgment.
...
Known band generators are labeled separately from Tony’s voice on catalog songs, the player, queue and request details, Backstage, lyric sheets, original prompts, Fear & Hunger, and mixtape tracks. **EMP** identifies paid Eleven Music compositions (the full name remains in the hover description and original prompt); **Local · ACE** identifies explicitly saved local requests. Older recordings without saved generator metadata are left unlabeled. Compact API and offline catalogs retain the same attribution.
...
- Remixed releases carry a **Remix** badge. Each grid/list card has a play/pause toggle synchronized with the persistent bottom player. The hero record shows a comic-book caption with real catalog lyrics when clicked and occasionally while the page is idle. Captions vary between the original single-line size and contiguous passages of up to four lines on phones, six on ordinary desktop viewports, or eight on taller screens.
...
- The landing catalog keeps every **Needs attention** request visible, then fills out at least three **On the way** entries with active work before waiting requests. Pending entries have title- or idea-inspired artwork and share the same continuous grid or aligned list as released songs. Their status and progress are visible at a glance; opening **Details** reveals the original idea, current stage, and request links. Pending work and published songs refresh every 30 seconds and on returning to the tab, preserving playback, open rows, and the reading position. Song filters apply to the playable catalog; pending rows follow the shared queue.
