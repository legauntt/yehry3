...
The **Listening activity** dashboard shows total listens, songs listened to, and the latest listen for every matching song across all pages. These are global listener statistics, including when viewing a profile’s Saved songs. The **Most listened to** button selects that ordering directly; the sort dropdown offers the same option and preserves existing `?sort=plays` links. The dashboard follows search, collection, and saved-song filters, and updates after recorded listening and automatic catalog refresh. Unavailable statistics show a dash rather than an invented zero.
