...
class CompositionEndingTests(unittest.TestCase):
    def test_preflight_uses_named_stage_for_every_voice_runtime(self):
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            save(work / 'desktop-status.json', {'stage': 'prepare',
                'completed': ['generate', 'separate', 'words', 'configure', 'ending']})
            commands = [
                ['python', str(work / 'convert_song.py'), 'prepare', '--skip-existing'],
                ['python', 'voice_runtime.py', str(work), 'prepare'],
                ['python', 'future_runtime.py', '--job', str(work), '--stage=prepare'],
            ]
            for command in commands:
                with self.subTest(command=command), patch('composition_ending.subprocess.Popen') as popen:
                    with self.assertRaises(CompositionReady): preflight_runner(command, cwd=work)
                    popen.assert_not_called()
            save(work / 'desktop-status.json', {'stage': 'ending', 'completed': []})
            with patch('composition_ending.subprocess.Popen') as popen:
                preflight_runner(['python', 'unrelated.py', 'prepare'], cwd=work)
                popen.assert_called_once()

    def test_preflight_refuses_to_reenter_conversion(self):
        with tempfile.TemporaryDirectory() as directory:
            work = Path(directory)
            save(work / 'desktop-status.json', {'stage': 'prepare', 'completed': ['prepare', 'features']})
            with patch('composition_ending.subprocess.Popen') as popen:
                with self.assertRaisesRegex(ValueError, 'after voice conversion'):
                    preflight_runner(['python', 'voice_runtime.py', str(work), 'prepare'], cwd=work)
                popen.assert_not_called()
