...
def build_references(base):
    ...
    for row in base.CONFIG['basis_sources']:
        ...
        if actual != row['sha256']: raise ValueError('A basis recording changed after planning')
        length = min(8., row['duration']); start = min(row['duration'] - length, row['duration'] * .25)
        raw = subprocess.check_output([base.FF, '-v', 'error', '-ss', str(max(0., start)), '-i', str(path),
        ...
        pieces.append(audio)
        clips.append({'title': row['title'], 'basis_id': row['id'], 'source_sha256': actual,
