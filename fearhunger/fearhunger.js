...
function update(songs) {
  ...
  for (const song of matching) {
    ...
    model.insertAdjacentHTML("afterend", musicBackendBadge(song) + remixBadge(song));
    ...
    const notice = qualityNotice(song.qualityIssues, song.reviewState, song.validationFailures, song.repairedAt);
