    ...
    await Promise.all(
      [...document.querySelectorAll("[data-dehaka-thread]")].filter((node) => !onScreenOnly || onScreen(node)).map(async (node) => {
        ...
        try {
          ...
          const html = dehakaThread(entries, { escape, date, canSteer: Boolean(node.closest("[data-prompt]")?.querySelector(".dehaka-form")) });
