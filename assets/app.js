  ...
  main.innerHTML = `
    <section class="hero">
      <div><p class="eyebrow">Tony C · The listening room</p><h1>Same voice.<br><em>Wildly different</em><br>directions.</h1>
        ...
        <div class="actions"><button class="primary" id="play-all">Play the collection <span aria-hidden="true">↗</span></button><a class="text-link" href="/longtimecomin/">Make a request <span aria-hidden="true">→</span></a></div>
      </div>
    ...
    <section class="request-banner"><p class="eyebrow">Long time comin’</p><h2>Heard something<br>in your head?</h2><p>Medusa as a barbershop quartet? An old favorite in a new universe? Put it on the wish list.</p><a class="primary" href="/longtimecomin/">Pitch the next song <span aria-hidden="true">↗</span></a></section>
...
async function admin() {
  ...
  function render() {
    main.innerHTML = `<section class="admin-intro"><div><p class="eyebrow">Backstage · Studio queue</p><h1>Make room for<br><em>the next one.</em></h1></div><button class="quiet" id="signout">Sign out ↗</button></section><div class="stats">${[
      ...
      .join(
        "",
      )}</select><button class="quiet" id="refresh">Refresh ↻</button><span class="small">${data.total} requests · Higher priority goes first; oldest wins ties.</span></div><div id="queue">${data.prompts.length ? data.prompts.map(row).join("") : '<div class="empty"><span class="empty-symbol">◎</span><h2>A little room for possibility.</h2><p>No requests in this view yet.</p><a class="text-link" href="/longtimecomin/">Make the first request →</a></div>'}</div><div class="pagination"><button class="quiet" id="prev-page" ${pageNumber === 0 ? "disabled" : ""}>← Previous</button><span>Page ${pageNumber + 1}</span><button class="quiet" id="next-page" ${(pageNumber + 1) * 50 >= data.total ? "disabled" : ""}>Next →</button></div></section>`;
