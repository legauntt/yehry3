    ...
    if (stage === "idea") {
      ...
      $("#confirm-form").onsubmit = (event) =>
        run(event, async () => {
          ...
          if (paid) rememberPaidPassword(paidPassword);
...
function brief(doc, showGpuWait = true) {
  return `${showGpuWait ? gpuWaitNotice(doc) : ""}${requestPromptBrief(doc, escape, voiceModelLabel)}${qualityNotice(doc.result?.qualityIssues || doc.qualityIssues, doc.reviewState || doc.result?.reviewState)}${doc.workerProgress ? `<p class="small">${escape(doc.workerProgress.stage)}${doc.status !== "failed" && doc.workerProgress.percent ? ` · ${Math.round(doc.workerProgress.percent)}%` : ""}</p>` : ""}${doc.workerError ? `<p class="field-error">${escape(doc.workerError)}</p>` : ""}`;
}
