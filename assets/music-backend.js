...
export function paidCost(details = {}) {
  const duration = details.generation?.duration || 240;
...
export function mountMusicBackend(root, { draft, generation, basisRoot, storage, api, escape, onChange }) {
  ...
  root.innerHTML = `<label for="music-backend">Band generator</label><select id="music-backend" name="musicBackend"><option value="local">Local · ACE (no music API charge)</option><option value="eleven_music" disabled>Eleven Music · paid</option></select><p class="small" id="music-backend-status" role="status">Checking paid music availability…</p><div id="paid-music-note" hidden><p class="small">Eleven Music composes the band and guide vocal, then the studio applies your selected Tony voice. Lyrics, style, instruments, timing and mix controls carry over. Basis recordings and local variation controls are unavailable.</p><p class="small" id="paid-music-cost"></p></div>`;
  ...
  const updateCost = () => {
    ...
    const duration = Number(field?.value || 240);
  ...
  api('/music-backends', { role: 'submitter' }).then((result) => {
    ...
    budget = result;
    available = result.enabled === true && !attached;
    select.querySelector('[value="eleven_music"]').disabled = !available;
    status.textContent = attached ? 'This request uses a recording. Choose local generation for catalog remixes.'
      : available ? 'Local is the default. Paid music is available to everyone who can submit requests.'
