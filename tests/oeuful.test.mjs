...
import { crateWeight, createPicker, wholeSong } from "../oeuful/crate.js";
...
test("Œuful never runs dry and rests a song before playing it again", () => {
  const clips = Array.from({ length: 40 }, (_, index) => clip("s" + index));
  const next = createPicker(clips, { random: dice() });
  const played = Array.from({ length: 800 }, () => next().id);
  for (let index = 0; index < played.length; index += 1)
    assert.ok(!played.slice(Math.max(0, index - 20), index).includes(played[index]), "A song came back within twenty records");
  assert.equal(new Set(played).size, 40, "Every song gets a turn");
});
...
test("Œuful plays liked and listened-to songs more often without losing the rest", () => {
  ...
  const next = createPicker(Array.from({ length: 60 }, (_, index) => clip("s" + index)), { random: dice(), weightOf: (picked) => crateWeight(songs.get(picked.id)) });
  const turns = new Map();
  for (let index = 0; index < 3000; index += 1) {
    const { id } = next();
    turns.set(id, (turns.get(id) || 0) + 1);
  }
  assert.equal(turns.size, 60, "Every song still gets a turn");
  const plain = (3000 - turns.get("s0") - turns.get("s1") - turns.get("s2")) / 57;
...
test("Œuful cuts its sides to the length each deck asks for", () => {
  ...
  const next = createPicker([{ id: "long", title: "Long", url: "https://example.test/long.mp3", lines, moments: [[0, 0]] }], { random: dice() });
  const lengths = (count, seconds) => Array.from({ length: count }, () => next(seconds)).map((moment) => moment.end - moment.start);
  for (const length of lengths(40, 8)) assert.equal(length, 6.5, "Two lines fit under eight seconds, and one alone is too short to be preferred");
  const longer = lengths(40, 20);
  ...
  assert.deepEqual(lengths(3, 2.5), [3, 3, 3]);
  // A deck that plays whole songs gets every line, first to last.
  const whole = next(wholeSong);
  assert.deepEqual([whole.start, whole.end, whole.lines.length], [10, 51.5, 12]);
});
