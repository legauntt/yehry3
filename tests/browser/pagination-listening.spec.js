...
test("123-song pagination supports deep links, history, filtering, sorting and mobile controls", async ({ page }) => {
  ...
  await page.goto("/?sort=catalog&page=6");
  await expect(page.locator(".track")).toHaveCount(3);
  await expect(page.locator(".track-number").first()).toHaveText("121");
  await expect(page.locator("#track-count")).toHaveText("123 songs · Showing 121–123");
  ...
  await expect(page.locator(".track-number").first()).toHaveText("97");
  await page.reload();
  await expect(page.locator(".track-number").first()).toHaveText("97");
  ...
  await expect(page.locator("#track-count")).toHaveText("60 songs · Showing 1–24");
  ...
  await expect(page.locator(".track-number").first()).toHaveText("25");
...
test("page changes and refresh preserve playback and seeking, and Next crosses the page boundary", async ({ page }) => {
  ...
  await expect(page.locator("#now-title")).toHaveText("Track 025");
  await page.locator(".catalog-filters > summary").click();
...
test("late live data and saved-profile loading retain requested pages; invalid pages are clamped", async ({ page }) => {
  ...
  await expect(page.locator(".track-number").first()).toHaveText("49");
